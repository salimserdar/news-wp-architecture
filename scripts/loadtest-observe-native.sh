...
#   INTERVAL=2 OUT=/tmp/observe.csv sudo -E bash scripts/loadtest-observe-native.sh
#
# Ctrl-C stops sampling; the CSV is flushed each row.
set -euo pipefail

INTERVAL="${INTERVAL:-2}"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
OUT="${OUT:-/tmp/news-wp-observe-${STAMP}.csv}"
LOG="${LOG:-/var/log/nginx/access.log}"
SLOW="${SLOW:-/var/log/php8.3-fpm-slow.log}"
SOCK="${SOCK:-/run/php/php8.3-fpm.sock}"

mkdir -p "$(dirname "$OUT")"
echo "ts,elapsed_s,new_lines,s2xx,s3xx,s4xx,s5xx,php_procs,php_rss_mb,load1,load5,mem_avail_mb,swap_used_mb,mysql_threads,fpm_active,fpm_idle,fpm_queue" > "$OUT"
...
if [[ -f "$LOG" ]]; then
  ...
  echo "warning: $LOG missing — status columns will stay 0 (wrong machine or nginx not running)" >&2
fi
...
echo "ts elapsed 2xx 4xx 5xx php_n load1 mem_av mysql_thr fpm_act/q" >&2
...
while true; do
  ...
  if [[ -f "$LOG" ]]; then
    ...
    if (( new_lines > 0 )); then
      st="$(tail -n "$new_lines" "$LOG" | count_status)"
    fi
  ...
  echo "$now,$elapsed,$new_lines,$st,$nphp,${rss:-0},$load1,$load5,$mem_avail,$swap_used,${mthr:-},$fpm" >> "$OUT"

  IFS=',' read -r s2 s3 s4 s5 <<< "$st"
  IFS=',' read -r fact fidle fq <<< "$fpm"
  printf '%s +%ss  2xx=%s 4xx=%s 5xx=%s  php=%s  load=%s  mem=%sMB  mysql=%s  fpm_act=%s q=%s\n' \
    "$now" "$elapsed" "$s2" "$s4" "$s5" "$nphp" "$load1" "$mem_avail" "${mthr:--}" "${fact:--}" "${fq:--}" >&2

  if [[ -f "$SLOW" ]]; then
    # print only new slow-log lines this interval (best-effort)
    :
  fi
  sleep "$INTERVAL"
done
