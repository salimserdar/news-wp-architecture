...
# Idempotent — re-run after a git pull to refresh nginx, PHP pool, MariaDB
# tuning, mu-plugins, and cache settings. It does not wipe WordPress files
# or the database.
#
...
CRED_FILE="/root/news-wp-credentials"
if [[ ! -f "${CRED_FILE}" && -f /root/news-wp-native-credentials ]]; then
  mv /root/news-wp-native-credentials "${CRED_FILE}"
  echo "    renamed /root/news-wp-native-credentials -> ${CRED_FILE}"
fi
...
sed -E "s/^pm.max_children = .*/pm.max_children = ${PHP_MAX_CHILDREN}/" \
  "${REPO_DIR}/config/php/pool-www.conf" \
  > /etc/php/8.3/fpm/pool.d/www.conf
# JIT is off: it hangs/kills wp-admin/load-styles.php with Newspaper.
grep -v -E '^[[:space:]]*opcache\.jit' "${REPO_DIR}/config/php/conf.d/zz-wp.ini" \
  > /etc/php/8.3/fpm/conf.d/99-wp.ini
cat >> /etc/php/8.3/fpm/conf.d/99-wp.ini <<'EOF'

; Native override: disable JIT (see pool-www.conf).
opcache.jit = disable
...
cp -a "${REPO_DIR}/config/nginx/http.conf" /etc/nginx/conf.d/00-news-wp.conf
cp -a "${REPO_DIR}/config/nginx/site.conf" /etc/nginx/sites-available/wordpress
...
umask 077
cat > "${CRED_FILE}" <<EOF
# Written by scripts/setup-vps.sh on $(date -u +%F)
SITE_DOMAIN=${SITE_DOMAIN}
DB_NAME=${DB_NAME}
DB_USER=${DB_USER}
...
echo
echo "Done (nginx FastCGI cache + PHP-FPM + MariaDB)."
...
echo "  WEB_OPEN=0 bash scripts/setup-vps.sh"
